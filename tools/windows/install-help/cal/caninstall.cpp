...
bool canInstall(BOOL isDC, int ddUserExists, int ddServiceExists, const CustomActionData &data, bool &bResetPassword)
{
    ...
    if (isDC)
    {
        ...
        if (!ddUserExists && (_wcsicmp(data.Domain().c_str(), data.GetTargetMachine().GetDomain().c_str())))
        {
            ...
            WcaLog(LOGMSG_STANDARD,
                   "(Configuration Error)  Can't create a user that's not in this domain: %S (asked for %S)",
