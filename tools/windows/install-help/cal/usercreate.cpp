...
#define CHARTYPE_SPECIAL 3
    ...
    do
    {
        ...
        times++;
    } while ((usedClasses[CHARTYPE_LOWER] < 2 || usedClasses[CHARTYPE_UPPER] < 2 || usedClasses[CHARTYPE_NUMBER] < 2 ||
              usedClasses[CHARTYPE_SPECIAL] < 2) ||
...
DWORD changeRegistryAcls(CustomActionData &data, const wchar_t *name)
{
    ...
    dduser.BuildGrantUser((SID *)usersid, GENERIC_ALL | KEY_ALL_ACCESS, SUB_CONTAINERS_AND_OBJECTS_INHERIT);
...
DWORD addDdUserPermsToFile(CustomActionData &data, std::wstring &filename)
{
    ...
    PSID usersid = GetSidForUser(NULL, data.Username().c_str());
    ExplicitAccess dduser;
    dduser.BuildGrantUser((SID *)usersid, FILE_ALL_ACCESS, SUB_CONTAINERS_AND_OBJECTS_INHERIT);
...
int doesUserExist(const CustomActionData &data, bool isDC)
{
    ...
    if (ERROR_INSUFFICIENT_BUFFER != err)
    {
        if (!isDC)
        {
            ...
            if (err >= ERROR_NO_TRUST_LSA_SECRET && err <= ERROR_TRUST_FAILURE)
            {
                ...
                if (data.isUserLocalUser() == NULL)
                {
                    ...
                    bRet = LookupAccountName(data.GetTargetMachine().GetMachineName().c_str(), data.Username().c_str(),
                                             newsid, &cbSid, refDomain, &cchRefDomain, &use);
                    if (bRet)
                    {
                        // this should *never* happen, because we didn't pass in a buffer large enough for
                        // the sid or the domain name.
                        WcaLog(LOGMSG_STANDARD, "doesUserExist: Lookup Account Name: Unexpected error %d 0x%x", err,
                               err);
                        return -1;
                    }
                    err = GetLastError();
                ...
                else
                {
                    WcaLog(LOGMSG_STANDARD,
                           "doesUserExist: Lookup Account Name: supplied domain, but can't check user database %d 0x%x",
                           err, err);
                    return -1;
                }
            }
        ...
        { // we don't know what happened
            ...
            WcaLog(LOGMSG_STANDARD,
                   "doesUserExist: Lookup Account Name: Expected insufficient buffer, got error %d 0x%x", err, err);
            return -1;
        }
    }
